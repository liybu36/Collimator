...
const Double_t veto_dt_cluster = 1e-7, veto_dr_cluster = 400, veto_dz_cluster = 400; //[secs] and [cm]                              
Float_t zshift = -1;
...
void reconSelector::Begin(TTree * /*tree*/)
{
   ...
   Info("Begin()","OutputFileName:%s ",option.Data());
}
...
void reconSelector::SlaveBegin(TTree * /*tree*/)
{
   ...
   TString option = GetOption();
   Info("slaveBegin()","OutputFileName: %s",option.Data());
   //   string outdir = "/cache/shared/darkside/hqian/TMBfraction/cluster_data/";
   //   string outdir="/darkside/users/hqian/TMBfraction/cluster_data/";
   //   string outdir="/darkside/users/hqian/VetoEfficiency/";
   //   string outdir="/darkside/users/hqian/60KeVGammaStudy/";
   //   string outdir="/darkside/users/hqian/pulse_splitter/MCData/";
   //   string outdir="/darkside/users/hqian/pulse_splitter/SDMCData/";
   string outdir="/darkside/users/hqian/AmC13/MCNeutron/";
   //   string outdir="/darkside/users/hqian/GammaCalibration/MCData/";
...
Bool_t reconSelector::Process(Long64_t entry)
{
  ...
  //  Info("Process()","! ! ! Start Parallel Processing Data =====>");
  FillHistograms(entry);
  //  TestFill(entry);
  //  Info("Process()","<======= End Parallel Processing Data !!!");
  return kTRUE;
}
...
void reconSelector::Terminate()
{
  ...
  //  delete ReconTree;
  Info("Terminate()","terminating ......Have Fun with the clustering data!!!");
  
}

void reconSelector::SetOutputTree(TFile *fFile)
{
  ...
  ReconTree->Branch("event_pdg",&event_pdg);
  //  ReconTree->Branch("event_broken",&event_broken);
  ReconTree->Branch("source_ene", &source_ene, 32000, 1);
  ReconTree->Branch("source_x", &source_x, 32000, 1);
  ReconTree->Branch("source_y", &source_y, 32000, 1);
  ReconTree->Branch("source_z", &source_z, 32000, 1);
  ReconTree->Branch("source_r", &source_r, 32000, 1);
  ReconTree->Branch("source_px", &source_px, 32000, 1);
  ReconTree->Branch("source_py", &source_py, 32000, 1);
  ReconTree->Branch("source_pz", &source_pz, 32000, 1);
  ...
  ReconTree->SetDirectory(fFile);
  ReconTree->AutoSave();
}

void reconSelector::TestFill(Long64_t event)
{
  ...
  */
      //  recon_et->push_back(event);  
  ReconTree->Fill();
}

//bool reconSelector::CompareRE(ReconEvent a, ReconEvent b)
bool CompareRE(ReconEvent a, ReconEvent b)
{
  return (a.et < b.et);
}

void reconSelector::InitializeRE(ReconEvent &evt, G4DSHitData &hit, int currentNum)
{
  ...
  if (hit.particle.at(currentNum) == "alpha" || hit.particle.at(currentNum) == "nucleus" || hit.particle.at(currentNum) == "neutron" || hit.particle.at(currentNum) == "proton")
...
bool reconSelector::CompatibleRE(ReconEvent evt, ReconEvent hit)
{
  //  if(hit.volume=="p_scint" && evt.volume=="p_scint" && fabs(hit.et-evt.et)<evt.dt)
  //    return true;
...
void reconSelector::MergeRE(ReconEvent &evt, ReconEvent hit)
{
  Double_t dist, temp;
  evt.edep += hit.edep;
  evt.eqch += hit.eqch;
...
double reconSelector::get_point(vector<double> xs, vector<double> ys, double x)
{
  double deltay;
  double deltax;
  if(x < xs.at(0))
...
double reconSelector::get_point(double *xs, double *ys, double x, int size)
{
  double deltay;
  double deltax;
  if(x < xs[0])
...
//333-338                                                                                                                            
double MeV = 1000.0;
double reconSelector::o_scint_e_quenching (double e/*keV*/)
{
  int k=3;
  //RNS                                                                                                                              
  ...
  const int N = 7;
  const int M = 6;
  double birks_value[N][M] = {{0.49041,0.21600,0.10725,-1.46804e-3,0.22468,0.09544}, //0.006 
  ...
  */
  return ( (A[0] + A[1]*TMath::Log(e) + A[2]*TMath::Log(e)*TMath::Log(e) + A[3]*TMath::Log(e)*TMath::Log(e)*TMath::Log(e))/
           (1 + A[4]*TMath::Log(e) + A[5]*TMath::Log(e)*TMath::Log(e) + A[3]*TMath::Log(e)*TMath::Log(e)*TMath::Log(e))  );
...
double o_scint_a_quenching_energy[] = {0,.7,.85,1,1.2,
                                       2,3,4,6,
                                       8,10,20,30};
double o_scint_a_quenching[] = {0,.017,.02,.025,.036,
                                .06,.15,.22,.5,
                                1,1.8,7,12};
...
double reconSelector::get_quenching_factor_organic_scintillator(ReconEvent evt)
{
...
double reconSelector::get_quenching_factor_liquid_argon(ReconEvent evt, bool fieldOn)
{
  string particle(evt.particle);
  string type(evt.type);
  string volume(evt.volume);
...
double reconSelector::get_quenching_factor (ReconEvent evt, bool fieldOn)
{
...
std::string reconSelector::convertPDG(int pdgNum){
...
bool reconSelector::checkActive(Float_t r, Float_t z){
...
std::string reconSelector::convertVolume(int mat , Float_t r, Float_t z){
...
std::string reconSelector::returnType(int pdgNum){
...
void reconSelector::EmptyHit(G4DSHitData &currentHit)
{
  currentHit.x.clear();
  currentHit.y.clear();
  currentHit.z.clear();
...
void reconSelector::returnHit(G4DSHitData &currentHit)
{
  EmptyHit(currentHit);
...
void reconSelector::FillHistograms(Long64_t event)
{
