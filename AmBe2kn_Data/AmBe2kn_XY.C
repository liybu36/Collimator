...
struct Par{
  Par():
    amp(0.),mean_f90(0.),sigma_f90(0.),
    mean_s2overs1(0.),sigma_s2overs1(0.)
  {}
  
  double amp;
  double mean_f90;
  double sigma_f90;
  double mean_s2overs1;
  double sigma_s2overs1;  

};

Double_t Gaus2D(Double_t *x, Double_t *par)
{
  Double_t res1 = TMath::Power((x[0]-par[1]),2)/(2*par[2]*par[2]);
  Double_t res2 = TMath::Power((x[1]-par[3]),2)/(2*par[4]*par[4]);
  return par[0]*TMath::Exp(-res1-res2)/(2*TMath::Pi()*par[2]*par[4]);
...
#define CorrXY
  ...
  for(size_t j=0; j<counts.size(); j++)
    {
      probs.at(j) = 1.0*counts.at(j)/nevents;
      cout<<j<<"  "<<counts.at(j)<<"   "<<probs.at(j)<<endl;	
    }
  ...
  if(outdata.is_open())
    {
      for(size_t j=0; j<counts.size(); j++)
	outdata<<j<<"  "<<counts.at(j)<<"   "<<probs.at(j)<<"\n";
    }
  gStyle->SetNumberContours(100);
  std::vector<Par> par;
  std::vector<double> f90power;
  std::vector<double> s2overs1power;
  for(int k=0; k<NUM; k++)
    {
      for(int i=1; i<3; ++i){
	par.push_back(Par());
	s2overs1_vs_f90_slice.at(k+i*NUM)->Sumw2();
	//	TF2 *f2 = new TF2("f2","[0]*TMath::Gaus(x,[1],[2])*TMath::Gaus(y,[3],[4])",0,1,0,3);
	TF2 *f2 = NULL;
	if(i==1){ //NR
	  f2 = new TF2("f2",Gaus2D,f90_cut,1,0,3,5);
	  f2->SetParameters(1,0.55,1,0.55,1);
	}else if(i==2){ //ER
	  f2 = new TF2("f2",Gaus2D,0,f90_cut,0,3,5);
	  f2->SetParameters(1,0.55,1,0.55,1);
	}
	s2overs1_vs_f90_slice.at(k+i*NUM)->Fit(f2,"RVEM");
	outdata<<k+i*NUM<<"\n";
	for(int h=0; h<5; h++)
	  outdata<<f2->GetParameter(h)<<" "<<f2->GetParError(h)<<"\n";
	par.back().amp     = f2->GetParameter(0);
	par.back().mean_f90  = f2->GetParameter(1);
	par.back().sigma_f90 = f2->GetParameter(2);
	par.back().mean_s2overs1  = f2->GetParameter(3);
	par.back().sigma_s2overs1 = f2->GetParameter(4);		
      }
      f90power.push_back(1.0*(par.at(k*2+1).mean_f90-par.at(k*2).mean_f90)/(TMath::Sqrt(TMath::Power(par.at(k*2+1).sigma_f90,2)+TMath::Power(par.at(k*2).sigma_f90,2))));
      s2overs1power.push_back(1.0*(par.at(k*2+1).mean_s2overs1-par.at(k*2).mean_s2overs1)/(TMath::Sqrt(TMath::Power(par.at(k*2+1).sigma_s2overs1,2)+TMath::Power(par.at(k*2).sigma_s2overs1,2))));      
  }

  outdata.close();    
  string note[3] = {"combined","F90","s2overs1"};
  TLegend *leg = new TLegend(0.5,0.7,0.8,0.9);
  vector<TH1F*> power_s1;
  for(int i=0; i<3; i++)
    {
      power_s1.push_back(new TH1F(Form("%s_power_s1",note[i].c_str()),"; total_s1_corr [PE]; discriminator power",NUM,start_s1*1.0,end_s1*1.0));
      power_s1.back()->SetLineColor(2+i);
      power_s1.back()->SetMarkerStyle(4);
      leg->AddEntry(power_s1.back(),note[i].c_str(),"l");  
    }
  const int M = 20;
  double s1_x[M], f90_y[M], s2overs1_y[M], combined_y[M];
  
  for(size_t j=0; j<f90power.size() && j<NUM; j++)
    {
      //      cout<<(low_s1.at(j)+high_s1.at(j))/2.<<endl;
      s1_x[j] = (low_s1.at(j)+high_s1.at(j))/2.;
      f90_y[j] = TMath::Sqrt(TMath::Power(f90power.at(j),2)+TMath::Power(s2overs1power.at(j),2));
      s2overs1_y[j] = TMath::Abs(f90power.at(j));
      combined_y[j] = TMath::Abs(s2overs1power.at(j));
      power_s1.at(0)->Fill((low_s1.at(j)+high_s1.at(j))/2.,TMath::Sqrt(TMath::Power(f90power.at(j),2)+TMath::Power(s2overs1power.at(j),2)));
      power_s1.at(1)->Fill((low_s1.at(j)+high_s1.at(j))/2.,TMath::Abs(f90power.at(j)));
      power_s1.at(2)->Fill((low_s1.at(j)+high_s1.at(j))/2.,TMath::Abs(s2overs1power.at(j)));      
    }
  canv.push_back(new TCanvas("power_s1_1","",600,400));
  canv.back()->cd();
  power_s1.at(0)->Draw();
  power_s1.at(1)->Draw("same");
  power_s1.at(2)->Draw("same");
  leg->Draw();

  TMultiGraph *mg = new TMultiGraph();
  mg->SetTitle("Discrimination Power Vs S1");
  //  mg->GetXaxis()->SetTitle("total_s1_corr [PE]");
  //  mg->GetYaxis()->SetTitle("discriminator power");

  canv.push_back(new TCanvas("power_s1_2","",600,400));
  canv.back()->cd();
  TGraph *g1 = new TGraph(M,s1_x,combined_y);
  TGraph *g2 = new TGraph(M,s1_x,f90_y);
  TGraph *g3 = new TGraph(M,s1_x,s2overs1_y);
  g1->SetLineColor(2);
  ...
  for(size_t j=0; j<power_s1.size(); j++)
    power_s1.at(j)->Write();
