...
//  }
  ...
  const Double_t electron_lifetime = SetElectronLifetime(run_id);//update from Guangyong on 02/03/2014//3338.;//updated to the value from Guangyong with adjusted laser window on 01/27/2014//3330; // � 62.4 //3076; //[us]
//  total_s2_corr = (run_id>=6553)? total_s2 : total_s2/TMath::Exp(-t_drift/electron_lifetime); // only apply S2 z_correction after run6553.
  total_s2_corr = total_s2/TMath::Exp(-t_drift/electron_lifetime); // only apply S2 z_correction after run6553.
...
//  Double_t total_s1_TBAcorr =  total_s1*(1.-diff_total_s1);
  ...
  if(total_s1_corr >= s1_min && total_s1_corr < s1_max) {
      ...
      ht_drift_S2XYCorroverS1_Kr->Fill(t_drift, S2XYcorrfac*s2_noncorr_s1_corr);//*scale_point/scale);
      ht_drift_ScaledS2XYCorroverS1_Kr->Fill(t_drift, S2XYcorrfac*s2_noncorr_s1_corr*scale_point/scale);
      ht_drift_S2XYCorr_Kr->Fill(t_drift, S2XYcorrfac*total_s2);//*scale_point/scale);
      ht_drift_S2AllCorr_Kr->Fill(t_drift, S2XYcorrfac*total_s2_corr);
  }

  if (s2_max_chan==30) ht_drift_S2overS1_center->Fill(t_drift, s2_noncorr_s1_corr);
...
//  pS2VsXY_Kr = new TProfile2D("pS2VsXY_Kr", "Averaged S2 vs xy. around Kr peak; x [cm]; y [cm]; total_s2_corr [PE]", 40, -20, 20, 40, -20, 20);//, 20, xmin, xmax, 20, ymin, ymax);//, 500, 0., 1.);
  ...
  ht_drift_ScaledS2XYCorroverS1_Kr = new TH2D("ht_drift_ScaledS2XYCorroverS1_Kr", "Scaled S2_{XYcorr. not z}/S1_{corr.} vs t_drift around Kr peak; t_drift [#mus]; total_s2_XYcorr_not_z/total_s1_corr", 100, minDriftT, maxDriftT, 500, 0, 100);
  list->Add(ht_drift_ScaledS2XYCorroverS1_Kr);
  ...
  ht_drift_S2AllCorr_Kr = new TH2D("ht_drift_S2AllCorr_Kr", "S2_{XYZcorr} vs t_drift around Kr peak; t_drift [#mus]; total_s2_XYZcorr [PE]", 100, minDriftT, maxDriftT, 300, 0, 30.e3);
  list->Add(ht_drift_S2AllCorr_Kr);

  ht_drift_S2overS1_center = new TH2D("ht_drift_S2overS1_center", "S2/S1_{corr.} vs t_drift w/ max_s2_chan = 30; t_drift [#mus]; total_s2/total_s1_corr", 100, minDriftT, maxDriftT, 500, 0, 100);
  list->Add(ht_drift_S2overS1_center);
...
Double_t SladReadSelector::SetElectronLifetime(Int_t RunId) {
