...
#define CorrXY
  ...
  for(int k=0; k<NUM; k++)
	...
	s2overs1_vs_f90_slice.at(k+i*NUM)->Fit(f2);
	outdata<<k+i*NUM<<"\n";
	for(int h=0; h<5; h++)
  ...
  for(size_t j=0; j<f90power.size(); j++)
    {
      power_s1.at(0)->Fill((low_s1.at(j)+high_s1.at(j))/2.,f90power.at(j)+s2overs1power.at(j));
      power_s1.at(1)->Fill((low_s1.at(j)+high_s1.at(j))/2.,f90power.at(j));
      power_s1.at(2)->Fill((low_s1.at(j)+high_s1.at(j))/2.,s2overs1power.at(j));      
    }
